...
FINAL_REPORT_PICTURE_FOLDER = f"{REPORT_PICTURE_FOLDER}/{formatted_date}"
...
def extract_year_month(file):
    date_part = file.split('_')[-1].replace('.xlsx', '')  # Get "202501"
    return pd.to_datetime(date_part, format="%Y%m")  # Convert to datetime
...
def generate_price_evolution_graph(bucket_name, folder_name, df, neighborhood):
    # Quitamos del dataframe los locales y oficinas
    # Dataframe negar la condición
    df = df[~df['Inmueble'].isin(["Oficina", "Local"])]
    df.drop(columns=['Inmueble'], inplace=True)
...
def generate_bar_charts_neighborhood(df, neighborhood, bucket_name, folder_name):
    ...
    if df_filtered.empty:
        print(f"No hay datos para el barrio {neighborhood}")
        return
...
print(f"El archivo mas nuevo es {newest_file}")
...
print(f"Borrando contenido de la carpeta {FINAL_REPORT_PICTURE_FOLDER}")
objects_to_delete = s3_client.list_objects_v2(Bucket=BUCKET_NAME, Prefix=FINAL_REPORT_PICTURE_FOLDER+"/")
...
print(f"Contenido de la carpeta {FINAL_REPORT_PICTURE_FOLDER} borrado")

# Obtenemos el archivo mas nuevo del historico
historic_files = s3_client.list_objects_v2(Bucket=BUCKET_NAME, Prefix=HISTORIC_FILE_FOLER)
sub_historic_files = list()
for file in historic_files["Contents"]:
    if (".xlsx" in file["Key"]):
        sub_historic_files.append(file["Key"])
sorted_historic_files = sorted(sub_historic_files, key=extract_year_month, reverse=True)
newest_historic_file = sorted_historic_files[0]

# Obtenemos el archivo mas nuevo del historico
response = s3_client.get_object(Bucket=BUCKET_NAME, Key=newest_historic_file)
excel_data = response['Body'].read()
df_historic = pd.read_excel(BytesIO(excel_data))


# Generamos los gráficos para cada barrio
for neighborhood in valid_neighborhoods:

    folder_name = f"{FINAL_REPORT_PICTURE_FOLDER}/{neighborhood}/"

    print(f"Generando gráficos para el barrio {neighborhood}")

    # Generamos los gráficos para cada barrio
    generate_price_m2_evolution_graph(BUCKET_NAME, folder_name, df_historic, neighborhood)
    generate_price_evolution_graph(BUCKET_NAME, folder_name, df_historic, neighborhood)
    generate_bar_charts(df_new, BUCKET_NAME, folder_name)
    generate_bar_charts_neighborhood(df_new, neighborhood, BUCKET_NAME, folder_name)
    generate_pie_charts(df_new, BUCKET_NAME, folder_name)
    generate_pie_charts_neighborhood(df_new, neighborhood, BUCKET_NAME, folder_name)
    generate_pie_charts_m2_neighborhood(df_new, neighborhood, BUCKET_NAME, folder_name)
